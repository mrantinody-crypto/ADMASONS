...
export function About() {
  ...
  return (
    <section id="about" className="py-28 bg-background relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-500/[0.02] via-transparent to-blue-500/[0.02]" />
      <div className="absolute top-1/2 -left-48 w-96 h-96 bg-blue-500/3 rounded-full blur-[150px]" />
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div ref={teamRef} className="mb-24">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {founders.map((founder, index) => (
              <div
                key={founder.name}
                className={`group relative bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 text-center transition-all duration-500 hover:border-blue-500/30 hover:-translate-y-3 hover:shadow-2xl hover:shadow-blue-500/15 ${teamInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              ...
              >
                ...
                <div className="relative w-24 h-24 mx-auto mb-6 rounded-2xl overflow-hidden shadow-lg group-hover:shadow-xl transition-all duration-500">
                  ...
                  {/* Gradient ring */}
                  <div className={`absolute inset-0 rounded-2xl ring-2 ring-white/10 group-hover:ring-blue-500/40 transition-all duration-500`} />
                </div>

                <h4 className="relative text-xl font-bold text-foreground mb-1">{founder.name}</h4>
                <p className="relative text-sm font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600 mb-3">{founder.role}</p>
        ...
        <div ref={extendedRef} className="mb-24">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4">
            {extendedTeamRoles.map((role, index) => (
              <div
                key={role.title}
                className={`group bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] rounded-xl p-5 text-center hover:border-blue-500/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300 ${extendedInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
                style={{ transitionDelay: extendedInView ? `${index * 80}ms` : "0ms" }}
              >
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500/15 to-blue-500/15 border border-blue-500/15 rounded-lg flex items-center justify-center mx-auto mb-3 group-hover:from-blue-500/30 group-hover:to-blue-500/30 transition-all duration-300">
        ...
        <div ref={philRef} className={`mb-24 transition-all duration-700 ${philInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="relative max-w-4xl mx-auto bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-3xl p-10 md:p-16 text-center overflow-hidden">
            ...
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-blue-500/5 via-transparent to-blue-500/5" />
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full" />

            <Sparkles className="w-8 h-8 text-blue-400 mx-auto mb-6 relative" />
        ...
        <div className="text-center">
          <a
            href="#contact"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold px-8 py-4 rounded-xl group shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 transition-all duration-300 hover:-translate-y-0.5"
            onClick={e => {
              e.preventDefault()
