...
const caseStudies = [
  ...
  },
  {
    ...
    gradient: "from-blue-600 to-primary/90",
  },
  {
  ...
  },
  {
    ...
    gradient: "from-violet-600 to-primary/90",
  },
]
...
export function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-28 bg-secondary/50 relative overflow-hidden">
      {/* Subtle background accent */}
      <div className="absolute bottom-0 right-0 w-80 h-80 bg-secondary0/3 rounded-full blur-[120px]" />
      <div className="absolute top-0 left-0 w-64 h-64 bg-secondary0/3 rounded-full blur-[100px]" />
      ...
      <div ref={sectionRef} className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {caseStudies.map((study, index) => (
            <div
              key={study.brand}
              className={`group relative overflow-hidden rounded-2xl bg-background border border-border/[0.06] hover:border-blue-500/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
            ...
            >
              ...
              <div className="aspect-[16/10] bg-secondary relative overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <span className="bg-background/80 backdrop-blur-sm text-xs font-semibold text-muted-foreground px-3 py-1.5 rounded-full border border-primary/20">
                    {study.industry}
                  </span>
                </div>
              </div>
        ...
        <div className={`mt-16 text-center transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: "700ms" }}>
          <a
            href="#contact"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-primary to-primary/90 text-white font-semibold px-8 py-4 rounded-xl shadow-lg shadow-primary/10/40 hover:shadow-blue-300/50 transition-all duration-300 hover:-translate-y-0.5 group"
            onClick={e => {
              e.preventDefault()
